...
import type { RegulationCategory, RegulationCategoriesResponse } from "@/core/domain/models/regulation-category"
import { FileText, Sparkles, Search } from "lucide-react"
import AdvancedBreadcrumbs from "@/components/advanced-breadcrumbs"

export default function RegulationsPage() {
      const { language, isRtl } = useLanguage()
      const [data, setData] = useState<RegulationCategoriesResponse | null>(null)
      ...
      useEffect(() => {
            const fetchData = async () => {
                  try {
                        setLoading(true)
                        const response = await container.services.regulationCategories.getAllCategories(1, 100)
                        setData(response)
                  } catch (error) {
      ...
      const filteredCategories = categories.filter(cat => {
            const name = language === "ar" ? cat.name : (cat.name_En || cat.name)
            ...
            return (
                  <div className="min-h-screen">
                        <div className="container mx-auto px-4 pt-8 pb-16">
                              <AdvancedBreadcrumbs items={[{ label: language === "ar" ? "اللوائح" : "Regulations" }]} />
                  ...
                  <div className="container mx-auto px-4 pt-8 pb-16">
                        ...
                        <div className="mb-10 mt-6">
                              ...
                              <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400 bg-clip-text text-transparent">
                                    {language === "ar" ? "اللوائح والأنظمة" : "Regulations & Policies"}
                              </h1>

                              <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl">
                                    {language === "ar"
                                          ? "تصفح اللوائح والأنظمة المتعلقة بالأمن السيبراني"
                                          : "Browse regulations and policies related to cybersecurity"}
                              </p>
                              ...
                              <div className="text-center py-16">
                                    <p className="text-xl text-gray-600 dark:text-gray-400">
                                          {searchQuery
                                                ...
                                                : (language === "ar" ? "لا توجد فئات لوائح متاحة" : "No regulation categories available")
                                          }
                                    </p>
                              </div>
                        ) : (
...
function CategoryCard({ category, language, isRtl }: { category: RegulationCategory; language: string; isRtl: boolean }) {
      const name = language === "ar" ? category.name : (category.name_En || category.name)

      return (
            <Link href={`/advanced/regulations/category/${category.id}`} className="group">
                  <article className="h-full bg-white/10 dark:bg-slate-800/50 rounded-2xl backdrop-blur-sm border border-white/20 dark:border-gray-700 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-2 p-6">
                        ...
                        <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-medium text-sm group-hover:gap-3 transition-all">
                              <span>{language === "ar" ? "تصفح اللوائح" : "Browse Regulations"}</span>
