...
import type { Regulation, RegulationsResponse } from "@/core/domain/models/regulation"
import type { RegulationCategory } from "@/core/domain/models/regulation-category"
import { FileText, Sparkles, Search, ChevronLeft, Calendar } from "lucide-react"
import AdvancedBreadcrumbs from "@/components/advanced-breadcrumbs"

export default function RegulationCategoryPage({ params }: { params: Promise<{ categoryId: string }> }) {
      ...
      useEffect(() => {
            const fetchData = async () => {
                  try {
                        ...
                        const cat = await container.services.regulationCategories.getCategoryById(resolvedParams.categoryId)
                        setCategory(cat)

                        // Fetch regulations for this category
                        const response = await container.services.regulations.getRegulationsByCategory(
      ...
      const filteredRegulations = regulations.filter(reg => {
            const title = language === "ar" ? (reg.title || reg.titleEn) : (reg.titleEn || reg.title)
            const summary = language === "ar" ? (reg.summary || reg.summaryEn) : (reg.summaryEn || reg.summary)
                  ...
                  <div className="container mx-auto px-4 pt-8 pb-16">
                        ...
                        <div className="mb-10 mt-6">
                              ...
                              <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl">
                                    {language === "ar"
                                          ? "تصفح اللوائح والأنظمة في هذه الفئة"
                                          : "Browse regulations and policies in this category"}
                              </p>
                              ...
                              {pagination && (
                                    <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-blue-100 dark:bg-blue-900/30 rounded-full">
                                          ...
                                          <span className="text-sm font-medium text-blue-700 dark:text-blue-300">
                                                {language === "ar" ? `${pagination.itemsCount} لائحة` : `${pagination.itemsCount} regulations`}
                                          </span>
                                    </div>
                              )}
                        </div>
                        ...
                        <div className="mb-8">
                              <div className="relative max-w-md">
                                    ...
                                    <input
                                          ...
                                          placeholder={language === "ar" ? "ابحث في اللوائح..." : "Search regulations..."}
                              ...
                              <>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                          {filteredRegulations.map((regulation) => (
                                                <RegulationCard key={regulation.id} regulation={regulation} language={language} isRtl={isRtl} />
                                          ))}
                                    </div>
...
function RegulationCard({ regulation, language, isRtl }: { regulation: Regulation; language: string; isRtl: boolean }) {
      ...
      return (
            <Link href={`/advanced/regulations/${regulation.id}`} className="group">
                  <article className="h-full min-h-[280px] bg-white/10 dark:bg-slate-800/50 rounded-2xl backdrop-blur-sm border border-white/20 dark:border-gray-700 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-2 flex flex-col">
                        {/* Image */}
                        {imageUrl ? (
                              <div className="relative h-36 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 overflow-hidden">
                                    <Image
                                          src={imageUrl}
                                          alt={title}
                                          fill
                              ...
                              <div className="h-36 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 flex items-center justify-center">
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                              ...
                              {/* Read More */}
                              <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-medium text-sm group-hover:gap-3 transition-all mt-3 pt-3 border-t border-gray-200/50 dark:border-gray-700/50">
