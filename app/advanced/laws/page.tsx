...
import type { LawCategory, LawCategoriesPaginatedResponse } from "@/core/domain/models/law-category"
import { Scale, Sparkles, Search } from "lucide-react"
import AdvancedBreadcrumbs from "@/components/advanced-breadcrumbs"

export default function LawsPage() {
      const { language, isRtl } = useLanguage()
      const [data, setData] = useState<LawCategoriesPaginatedResponse | null>(null)
      ...
      useEffect(() => {
            const fetchData = async () => {
                  try {
                        setLoading(true)
                        const response = await container.services.laws.getAllCategories(1, 100)
                        setData(response)
                  } catch (error) {
            ...
            return (
                  <div className="min-h-screen">
                        <div className="container mx-auto px-4 pt-8 pb-16">
                              <AdvancedBreadcrumbs items={[{ label: language === "ar" ? "القوانين" : "Laws" }]} />
                  ...
                  <div className="container mx-auto px-4 pt-8 pb-16">
                        ...
                        <div className="mb-10 mt-6">
                              ...
                              <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400 bg-clip-text text-transparent">
                                    {language === "ar" ? "القوانين والتشريعات" : "Laws & Legislation"}
                              </h1>

                              <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl">
                                    {language === "ar"
                                          ? "تصفح القوانين والتشريعات المتعلقة بالأمن السيبراني"
                                          : "Browse laws and legislation related to cybersecurity"}
                              </p>
                              ...
                              <div className="text-center py-16">
                                    <p className="text-xl text-gray-600 dark:text-gray-400">
                                          {searchQuery
                                                ...
                                                : (language === "ar" ? "لا توجد فئات قوانين متاحة" : "No law categories available")
                                          }
                                    </p>
                              </div>
                        ) : (
...
function CategoryCard({ category, language, isRtl }: { category: LawCategory; language: string; isRtl: boolean }) {
      const name = language === "ar" ? category.name : (category.nameEn || category.name)

      return (
            <Link href={`/advanced/laws/category/${category.id}`} className="group">
                  <article className="h-full bg-white/10 dark:bg-slate-800/50 rounded-2xl backdrop-blur-sm border border-white/20 dark:border-gray-700 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-2 p-6">
                        ...
                        <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-medium text-sm group-hover:gap-3 transition-all">
                              <span>{language === "ar" ? "تصفح القوانين" : "Browse Laws"}</span>
