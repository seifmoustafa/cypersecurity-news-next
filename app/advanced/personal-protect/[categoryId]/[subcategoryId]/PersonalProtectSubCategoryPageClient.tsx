...
interface PersonalProtectSubCategoryPageClientProps {
  subCategory: {
    id: string
    name: string
    nameEn: string
...
export default function PersonalProtectSubCategoryPageClient({ subCategory, categoryId }: PersonalProtectSubCategoryPageClientProps) {
  ...
  useEffect(() => {
    const fetchControls = async () => {
      try {
        const controlsData = await container.services.personalProtect.getControlsBySubCategoryId(subCategory.id, 1, 100)
  ...
  const title = language === "ar" ? subCategory.name : subCategory.nameEn
  const description = language === "ar" ? subCategory.description : subCategory.descriptionEn
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-6">
              <Link href={`/advanced/personal-protect/${categoryId}`}>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {controls.map((control) => (
                  <Link key={control.id} href={`/advanced/personal-protect/${categoryId}/${subCategory.id}/${control.id}`}>
                    <Card className="hover:shadow-lg transition-shadow cursor-pointer">
                      <CardContent className="p-6">
                        ...
                        <h3 className="font-bold mb-2 line-clamp-2">
                          {language === "ar" ? control.name : control.nameEn}
                        </h3>
                        <p className="text-sm text-muted-foreground line-clamp-3">
                          {language === "ar" ? control.description : control.descriptionEn}
                        </p>
                      </CardContent>
                    </Card>
                  </Link>
