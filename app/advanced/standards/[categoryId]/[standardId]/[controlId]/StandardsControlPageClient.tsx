...
export default function StandardsControlPageClient({ control, categoryId, standardId }: StandardsControlPageClientProps) {
  ...
  useEffect(() => {
    const fetchSafeguards = async () => {
      try {
        const safeguardsData = await container.standardsService.getSafeguardsByControlId(control.id, 1, 100)
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-6">
              <Link href={`/standards/${categoryId}/${standardId}`}>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {safeguards.map((safeguard) => (
                  <Link key={safeguard.id} href={`/standards/${categoryId}/${standardId}/${control.id}/${safeguard.id}`}>
