...
export default function FrameworkPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-8 pb-16 flex justify-center items-center">
    ...
    return (
      <div className="min-h-screen pt-8 pb-16 flex justify-center items-center">
        <div className="text-center max-w-2xl mx-auto px-4">
          ...
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl p-8 border border-red-200/30 dark:border-red-800/30 shadow-lg">
            ...
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-semibold rounded-xl transition-all duration-300 shadow-lg"
            >
    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 pt-8">
        ...
        <AdvancedBreadcrumbs items={[
          { label: language === "ar" ? "إطار العمل" : "Framework" }
        ]} />
      </div>
      <div className="relative container mx-auto px-4">
        ...
        <div className="mb-16">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            {frameworkFunctions.map((func) => (
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
                className={`cursor-pointer p-4 rounded-lg ${activeFunction === func.id
          ...
          {frameworkFunctions.map((func) => (
            <div
              key={func.id}
              id={func.id}
              className={`mb-12 scroll-mt-24 ${activeFunction === func.id ? "opacity-100" : "opacity-70"
        ...
        <div className="mb-16">
          ...
          <div className="relative" dir={isRtl ? "rtl" : "ltr"}>
            {/* Vertical line connecting steps - on left in LTR, on right in RTL */}
            <div className={`absolute top-0 bottom-0 w-0.5 bg-primary hidden md:block ${isRtl ? 'right-4' : 'left-4'}`}></div>
            ...
            {implementationSteps.map((step, index) => (
              ...
              >
                {/* Flex with proper direction */}
                <div className="flex items-start">
                  {/* Step number circle */}
                  <div className={`hidden md:flex items-center justify-center w-8 h-8 rounded-full bg-primary text-white font-bold z-10 flex-shrink-0 ${isRtl ? 'ml-4' : 'mr-4'}`}>
                    {step.step}
                  </div>
                  {/* Card content */}
                  <Card className="flex-1">
                    <CardHeader>
                      <div className="flex items-center">
                        {/* Mobile step number */}
                        <div className={`md:hidden flex items-center justify-center w-8 h-8 rounded-full bg-primary text-white font-bold flex-shrink-0 ${isRtl ? 'ml-4' : 'mr-4'}`}>
                          {step.step}
                        </div>
              ...
              >
                <Card className="h-full">
                  <CardHeader className="pb-2">
                    <CardTitle
                      className={`text-xl flex items-center gap-2 ${isRtl ? "text-right" : "justify-start"
                        }`}
                    >
