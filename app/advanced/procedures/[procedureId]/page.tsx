...
import type { Procedure, ProcedureControlsPaginatedResponse } from "@/core/domain/models/procedure"
...
export default function ProcedureDetailPage({ params }: ProcedurePageProps) {
      ...
      const [controlsData, setControlsData] = useState<ProcedureControlsPaginatedResponse | null>(null)
      ...
      useEffect(() => {
            const fetchData = async () => {
                  try {
                        ...
                        // Fetch controls for this procedure
                        const controls = await container.services.procedures.getControlsByProcedureId(resolvedParams.procedureId, 1, 100, searchQuery)
            ...
            fetchData()
      }, [resolvedParams.procedureId, searchQuery])

      const procedureName = procedure ? (language === "ar" ? (procedure.nameAr || procedure.nameEn) : (procedure.nameEn || procedure.nameAr)) : ""
      const procedureDescription = procedure ? (language === "ar" ? (procedure.descriptionAr || procedure.descriptionEn) : (procedure.descriptionEn || procedure.descriptionAr)) : ""
      ...
      return (
            <div className="min-h-screen">
                  <div className="container mx-auto px-4 pt-8 pb-16">
                        {/* Breadcrumbs: Home > Procedures > [Procedure] */}
                        <AdvancedBreadcrumbs items={[
                              { label: language === "ar" ? "الإجراءات" : "Procedures", href: "/advanced/procedures" },
                              { label: procedureName || "" }
                        ]} />
                        ...
                        <div className="mb-10 mt-6">
                              ...
                              {procedureDescription && (
                                    <p className="text-lg text-gray-600 dark:text-gray-300 max-w-3xl whitespace-pre-line">
                                          {procedureDescription.replace(/<\/?[^>]+(>|$)/g, "").trim()}
                                    </p>
                              )}
                              ...
                              <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-blue-100 dark:bg-blue-900/30 rounded-full">
                                    ...
                                    <span className="text-sm font-medium text-blue-700 dark:text-blue-300">
                                          {language === "ar" ? `${controls.length} عنصر تحكم` : `${controls.length} controls`}
                                    </span>
                              </div>
                        </div>
                        ...
                        <div className="mb-8">
                              <div className="relative max-w-md">
                                    ...
                                    <input
                                          ...
                                          placeholder={language === "ar" ? "ابحث في عناصر التحكم..." : "Search controls..."}
...
function ControlCard({ control, procedureId, language, isRtl }: { control: any; procedureId: string; language: string; isRtl: boolean }) {
      const name = language === "ar" ? (control.nameAr || control.nameEn) : (control.nameEn || control.nameAr)
      ...
      return (
            <Link href={`/advanced/procedures/${procedureId}/${control.id}`} className="group">
                  <article className="h-full bg-white/10 dark:bg-slate-800/50 rounded-2xl backdrop-blur-sm border border-white/20 dark:border-gray-700 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-2 p-6">
                        ...
                        <div className="flex items-start justify-between mb-4">
                              ...
                              {control.code && (
                                    <div className="px-3 py-1 bg-blue-100 dark:bg-blue-900/30 rounded-full">
                                          <span className="text-xs font-mono font-medium text-blue-600 dark:text-blue-400">{control.code}</span>
                                    </div>
                              )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-medium text-sm group-hover:gap-3 transition-all">
                              <span>{language === "ar" ? "عرض الضمانات" : "View Safeguards"}</span>
