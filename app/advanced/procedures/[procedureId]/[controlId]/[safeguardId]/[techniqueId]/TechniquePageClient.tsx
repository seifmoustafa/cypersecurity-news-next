...
export default function TechniquePageClient({
  technique,
  procedureId,
  controlId,
  safeguardId,
}: TechniquePageClientProps) {
  ...
  const { items: breadcrumbItems } = useSimpleAdvancedBreadcrumbs([
    ...
    { label: technique.nameEn || technique.nameAr, labelAr: technique.nameAr || technique.nameEn },
  ]);
  ...
  useEffect(() => {
    const fetchImplementationSteps = async () => {
      try {
        const stepsData =
          await container.services.procedures.getImplementationStepsByTechniqueId(
            technique.id,
            1,
            100
          );
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {implementationSteps.map((step) => (
                  <Link
                    key={step.id}
                    href={`/advanced/procedures/${procedureId}/${controlId}/${safeguardId}/${technique.id}/${step.id}`}
                  >
