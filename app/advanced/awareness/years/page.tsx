...
import type { AwarenessYearResponse } from "@/core/domain/models/awareness"
import { Calendar, Folder, TrendingUp, Sparkles } from "lucide-react"
import AdvancedBreadcrumbs from "@/components/advanced-breadcrumbs"
import Link from "next/link"

export default function AwarenessYearsPage() {
  const { language, isRtl } = useLanguage()
  const [data, setData] = useState<AwarenessYearResponse | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("❌ Error fetching awareness years:", error)
      } finally {
        setLoading(false)
      }
    }
      ...
      <div className="min-h-screen">
        <div className="container mx-auto px-4 pt-8 pb-16">
          <AdvancedBreadcrumbs items={[
            ...
            { label: language === "ar" ? "جميع السنوات" : "All Years" }
          ]} />
  ...
  return (
    <div className="min-h-screen">
      <div className="container mx-auto px-4 pt-8 pb-16">
        {/* Breadcrumbs: Home > Awareness > All Years */}
        <AdvancedBreadcrumbs items={[
          { label: language === "ar" ? "التوعية" : "Awareness", href: "/advanced/awareness" },
          { label: language === "ar" ? "جميع السنوات" : "All Years" }
        ]} />
        ...
        <div className="mb-10 mt-6">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400 bg-clip-text text-transparent">
            {language === "ar" ? "أرشيف نشرات التوعية" : "Awareness Bulletin Archive"}
          </h1>
          
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl">
            {language === "ar" 
              ? "تصفح نشرات التوعية حسب السنة"
              : "Browse awareness bulletins by year"}
          </p>
        ...
        {years.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {years.map((yearItem) => (
              <YearCard key={yearItem.id} yearItem={yearItem} language={language} isRtl={isRtl} />
            ))}
          </div>
        )}
      </div>
...
function YearCard({ yearItem, language, isRtl }: { yearItem: any; language: string; isRtl: boolean }) {
  return (
    <Link href={`/advanced/awareness/${yearItem.year}`} className="group">
      <article className="h-full bg-white/10 dark:bg-slate-800/50 rounded-2xl backdrop-blur-sm border border-white/20 dark:border-gray-700 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:-translate-y-2 p-6 text-center">
        ...
        <h3 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
          {language === "ar" ? `نشرات ${yearItem.year}` : `${yearItem.year} Bulletins`}
        </h3>
