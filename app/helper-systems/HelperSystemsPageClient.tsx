...
export default function HelperSystemsPageClient({ initialHelperSystems }: HelperSystemsPageClientProps) {
  const { language, isRtl } = useLanguage()
  const router = useRouter()
  const searchParams = useSearchParams()

  const [helperSystems, setHelperSystems] = useState<HelperSystem[]>(initialHelperSystems.data || [])
  ...
  const loadHelperSystems = async (page: number, search?: string) => {
    try {
      setLoading(true)
      const response = await container.services.helperSystems.getHelperSystems(page, 12, search)
      ...
      const newUrl = `/helper-systems${params.toString() ? `?${params.toString()}` : ""}`
  ...
  const getDisplayName = (system: HelperSystem) => {
    return language === "ar" ? system.name || system.nameEn || "" : system.nameEn || system.name || ""
  }
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="mb-8">
            <div className="relative max-w-md mx-auto">
              ...
              <Input
                type="text"
                placeholder={language === "ar" ? "البحث في الأنظمة المساعدة..." : "Search helper systems..."}
                value={searchTerm}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {helperSystems.map((system) => {
                ...
                return (
                  <Card key={system.id} className="hover:shadow-lg transition-all duration-300 group">
                    <CardContent className="p-6">
                      {system.iconUrl && (
                      ...
                      <Button
                        onClick={() => handleDownload(system.downloadUrl, getDisplayName(system))}
