...
import { Shield, Search, Calendar, ArrowRight, ArrowLeft, Eye, Users, Lock, Home } from "lucide-react"
import Link from "next/link"
import { useMemo, useState } from "react"
import Breadcrumbs from "@/components/breadcrumbs"

interface PersonalProtectSubCategoriesPageProps {
  params: Promise<{
    id: string
  }>
}

export default function PersonalProtectSubCategoriesPage({ params }: PersonalProtectSubCategoriesPageProps) {
  ...
  const { subCategories, loading, error, pagination } = usePersonalProtectSubCategories(categoryId, query, currentPage, pageSize)
  
  // Get category info for breadcrumbs and title
        ...
        <div className="container mx-auto px-4 pt-24 pb-8">
          <Breadcrumbs 
            items={[
              ...
              { label: language === "ar" ? "الفئات الفرعية" : "Sub Categories" }
            ]} 
          />
        ...
        <div className="container mx-auto px-4 pt-24 pb-8">
          ...
          <div className="max-w-4xl mx-auto text-center py-16">
            ...
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">
              {language === "ar" ? "خطأ في تحميل الفئات الفرعية" : "Error Loading Sub Categories"}
            </h1>
            <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
              {language === "ar" 
                ? "حدث خطأ أثناء تحميل الفئات الفرعية. يرجى المحاولة مرة أخرى."
                : "An error occurred while loading sub categories. Please try again."
              }
            </p>
            <button 
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-slate-950 dark:via-gray-950 dark:to-slate-900">
      ...
      <div className="relative z-10 container mx-auto px-4 pt-24 pb-8">
        ...
        <Breadcrumbs 
          items={[
            { label: language === "ar" ? "الحماية الشخصية" : "Personal Protection", href: "/simple/personal-protect" },
            { label: categoryName || (language === "ar" ? "الفئات الفرعية" : "Sub Categories") }
          ]} 
        />
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  {language === "ar" ? "الفئات الفرعية للحماية الشخصية" : "Personal Protection Sub Categories"}
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={language === "ar" ? "ابحث في الفئات الفرعية..." : "Search sub categories..."}
        ...
        {pagination && (
          <div className="mb-6">
            <p className="text-gray-600 dark:text-gray-400">
              {language === "ar" 
                ? `تم العثور على ${pagination.itemsCount} فئة فرعية`
                : `Found ${pagination.itemsCount} sub categories`
              }
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {subCategories.map((subCategory, index) => {
            const displayName = language === "ar" ? subCategory.name : (subCategory.nameEn || subCategory.name)
            const displayDescription = language === "ar" ? subCategory.description : (subCategory.descriptionEn || subCategory.description)
            
            return (
              <Link 
                key={subCategory.id} 
                href={`/simple/personal-protect/${categoryId}/${subCategory.id}`} 
                className="group"
                ...
                >
                  ...
                  <div className="relative aspect-video bg-gradient-to-br from-green-500 to-emerald-600">
                    ...
                    <div className="absolute top-4 right-4 bg-black/50 text-white text-xs px-3 py-1 rounded-full">
                      {language === "ar" ? "فئة فرعية" : "Sub Category"}
                    </div>
                  </div>
                  ...
                  <div className="p-8">
                    ...
                    <div className="inline-flex items-center justify-center w-full py-3 px-6 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-semibold rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-lg group/btn focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white/10 focus:ring-green-400">
                      <span className="mr-2 rtl:mr-0 rtl:ml-2">
                        {language === "ar" ? "عرض الفئة الفرعية" : "View Sub Category"}
                      </span>
                      {isRtl ? (
        ...
        {subCategories.length === 0 && !loading && (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
              {language === "ar" ? "لا توجد فئات فرعية" : "No Sub Categories Found"}
            </h3>
            <p className="text-gray-600 dark:text-gray-400">
              {language === "ar" 
                ? "لم يتم العثور على أي فئات فرعية تطابق البحث الخاص بك."
