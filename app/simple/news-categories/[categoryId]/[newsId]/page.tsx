...
import { useNewsBreadcrumbs } from "@/hooks/use-breadcrumbs"
...
export default function NewsDetailPage({ params }: NewsDetailPageProps) {
  ...
  const { items: breadcrumbItems, isLoading: breadcrumbLoading } = useNewsBreadcrumbs(resolvedParams.newsId, resolvedParams.categoryId)
  
  const { news, loading, error } = useNewsItem(resolvedParams.newsId)
  
  // Fetch related news from the same category (excluding current news)
  const { news: relatedNews } = useNews(resolvedParams.categoryId, 1, 5)
  const filteredRelatedNews = relatedNews.filter(item => item.id !== resolvedParams.newsId).slice(0, 4)
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="max-w-4xl mx-auto text-center py-16">
            ...
            <Link 
              href={`/simple/news-categories/${resolvedParams.categoryId}`}
              ...
              <div className="bg-gradient-to-r from-blue-50 to-sky-50 dark:from-slate-800 dark:to-slate-700 rounded-2xl p-8 border border-blue-200 dark:border-blue-800">
                <div className="flex items-center justify-between">
                  ...
                  <ShareButton
                    ...
                    text={language === "ar" ? `خبر: ${displayTitle}` : `News: ${displayTitle}`}
                  ...
                  <div className="space-y-4">
                    {filteredRelatedNews.map((relatedItem, index) => {
                      ...
                      return (
                        <Link
                          key={relatedItem.id}
                          href={`/simple/news-categories/${resolvedParams.categoryId}/${relatedItem.id}`}
