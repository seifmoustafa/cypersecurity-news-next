...
export default function NewsCategoryPage({ params }: NewsCategoryPageProps) {
  ...
  const { news, loading, error } = useNews(resolvedParams.id, 1, 100, debouncedQuery)
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-slate-950 dark:via-gray-950 dark:to-slate-900 flex items-center justify-center">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-muted-foreground mb-2">
            {language === "ar" ? "حدث خطأ في تحميل الأخبار" : "Error loading news"}
          </h3>
      ...
      <div className="relative z-10 container mx-auto px-4 pt-24 pb-8">
        ...
        <Breadcrumbs 
          items={[
            { label: language === "ar" ? "التوعية والأخبار" : "Awareness & News", href: "/simple/awareness" },
            { label: language === "ar" ? "فئات الأخبار" : "News Categories", href: "/simple/news-categories" },
            { label: language === "ar" ? "أخبار الفئة" : "Category News" }
          ]} 
        />
        ...
        {news.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-muted-foreground mb-2">
              {debouncedQuery 
                ? (language === "ar" ? "لا توجد أخبار تطابق البحث" : "No news match your search")
            ...
            <p className="text-muted-foreground">
              {debouncedQuery
                ...
                : (language === "ar"
                    ? "لم يتم العثور على أي أخبار في هذه الفئة"
                ...
                >
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex items-center mb-6">
                      ...
                      <div>
                        <div className="text-sm text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-2">
                          <Calendar className="h-4 w-4" /> {new Date(newsItem.date || newsItem.createdAt).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed mb-6 line-clamp-4">
                      {language === "ar" ? newsItem.summary : newsItem.summaryEn || newsItem.summary || "No summary available"}
                    </div>
