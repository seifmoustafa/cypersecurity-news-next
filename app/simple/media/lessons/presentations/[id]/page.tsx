...
import { Presentation, ArrowRight, ArrowLeft, Star, BookOpen, Search, Calendar } from "lucide-react"
import Breadcrumbs from "@/components/breadcrumbs"
import { usePresentationsByCategory } from "@/core/hooks/use-presentations-by-category"
import { usePresentationCategories } from "@/core/hooks/use-presentation-categories"
import { useDebounce } from "@/hooks/use-debounce"

interface PresentationCategoryPageProps {
  params: Promise<{
    id: string
  }>
}

export default function PresentationCategoryPage({ params }: PresentationCategoryPageProps) {
  ...
  const { presentations, loading, error } = usePresentationsByCategory(resolvedParams.id, 1, 100, debouncedQuery)
  const { categories: allCategories } = usePresentationCategories(1, 100)
  
  // Find the current category name
  const currentCategory = allCategories.find(cat => cat.id === resolvedParams.id)
  const categoryName = currentCategory ? (language === "ar" ? currentCategory.name : currentCategory.nameEn || currentCategory.name) : (language === "ar" ? "فئة العروض التقديمية" : "Presentation Category")
            ...
            <div className="flex items-center gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder={language === "ar" ? "ابحث في العروض التقديمية..." : "Search presentations..."}
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-slate-950 dark:via-gray-950 dark:to-slate-900 flex items-center justify-center">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-muted-foreground mb-2">
            {language === "ar" ? "حدث خطأ في تحميل العروض التقديمية" : "Error loading presentations"}
          </h3>
      ...
      <div className="relative z-10 container mx-auto px-4 pt-24 pb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {presentations.map((presentation, index) => (
            <Link 
              key={presentation.id} 
              href={`/simple/media/lessons/presentations/${resolvedParams.id}/${presentation.id}`} 
              className="group"
              ...
              >
                ...
                <div className="p-8">
                  ...
                  <div className="flex items-center mb-6">
                    <div className="bg-gradient-to-r from-purple-500 to-pink-600 p-3 rounded-xl mr-4 rtl:mr-0 rtl:ml-4 group-hover:scale-110 transition-transform duration-500 shadow-lg">
                      <Presentation className="h-6 w-6 text-white" />
                    </div>
                    <div>
                      <div className="text-sm text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-2">
                        <Calendar className="h-4 w-4" /> {new Date(presentation.createdAt).toLocaleDateString()}
                      </div>
                    </div>
                  </div>

                  {/* Presentation Title */}
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors duration-300 line-clamp-2">
                  ...
                  <div className="inline-flex items-center justify-center w-full py-3 px-6 bg-gradient-to-r from-purple-500 to-pink-600 text-white font-semibold rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-lg group/btn focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white/10 focus:ring-purple-400">
                    <span className="mr-2 rtl:mr-0 rtl:ml-2">
                      {language === "ar" ? "عرض العرض التقديمي" : "View Presentation"}
                    </span>
                    {isRtl ? (
