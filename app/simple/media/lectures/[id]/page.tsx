...
import { GraduationCap, ArrowRight, ArrowLeft, Star, BookOpen, Search, FileText, Clock, Calendar } from "lucide-react"
import Breadcrumbs from "@/components/breadcrumbs"
import { useLecturesByCategory } from "@/core/hooks/use-lectures-by-category"
import { useLectureCategories } from "@/core/hooks/use-lecture-categories"
import { useDebounce } from "@/hooks/use-debounce"

interface LectureCategoryPageProps {
  params: Promise<{
    id: string
  }>
}

export default function LectureCategoryPage({ params }: LectureCategoryPageProps) {
  ...
  const { lectures, loading, error } = useLecturesByCategory(resolvedParams.id, 1, 100, debouncedQuery)
  const { categories: allCategories } = useLectureCategories(1, 100)
  
  // Find the current category name
  const currentCategory = allCategories.find(cat => cat.id === resolvedParams.id)
  const categoryName = currentCategory ? (language === "ar" ? currentCategory.name : currentCategory.nameEn || currentCategory.name) : (language === "ar" ? "فئة المحاضرات" : "Lecture Category")
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 dark:from-slate-950 dark:via-gray-950 dark:to-slate-900 flex items-center justify-center">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-muted-foreground mb-2">
            {language === "ar" ? "حدث خطأ في تحميل المحاضرات" : "Error loading lectures"}
          </h3>
