...
export default function BeginnersSitemapPage() {
  ...
  const { categories: newsCategories, loading: newsLoading } =
    useNewsCategories(1, 100);
  const { categories: definitionCategories, loading: definitionLoading } =
    useDefinitionCategories(1, 100);
  const { categories: videoCategories, loading: videoLoading } =
    useVideoCategories(1, 100);
  const { categories: lectureCategories, loading: lectureLoading } =
    useLectureCategories(1, 100);
  const { categories: helperCategories, loading: helperLoading } =
    useHelperCategories(1, 100);
  const { data: awarenessYearsData, loading: awarenessYearsLoading } =
    useAwarenessYears();
  ...
  const sitemapData: TreeNode[] = [
    ...
    },
    {
      ...
      description:
        language === "ar"
          ? "فيديوهات  للحماية الشخصية"
          : "Educational videos for personal protection",
      type: "branch",
      level: 1,
      color: "text-blue-500",
...

    {
      ...
      children: [
        ...
        },
        {
          ...
          color: "text-cyan-500",
          children:
            awarenessYearsData?.data?.map((year: any) => ({
      ...
      children: [
        {
          ...
          children: lectureCategories.map((category) => ({
            ...
            description:
              language === "ar"
                ? `محاضرات ${category.name}`
                : `Lectures for ${category.nameEn || category.name}`,
            type: "leaf",
            level: 2,
            color: "text-cyan-400",
          })),
        ...
        },
        {
          ...
          description:
            language === "ar" ? "مراجع علمية" : "Scientific references",
          type: "leaf",
          level: 1,
                ...
                {isHovered && (
                  <div
                    className={`absolute inset-0 rounded-lg bg-gradient-to-r ${
                      node.gradient || "from-blue-500 to-cyan-500"
                    } opacity-20 blur-sm`}
                  />
                )}
              </div>
  ...
  const renderListView = () => {
    const renderNodeInList = (node: TreeNode, level: number = 0) => {
      ...
      return (
        <div
          key={node.id}
          className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border-2 border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-600 transition-colors overflow-hidden"
        >
          <div
            className={`${
              hasGradient
