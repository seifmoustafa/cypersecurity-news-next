...
export default function CurrentYearAwarenessPage() {
  ...
  const { awareness: awarenessData, loading, error } = useCurrentYearAwareness(debouncedQuery, 1, 100)
  const { data: yearsData } = useAwarenessYears("", 1, 50)
  const instructions = awarenessData || []
  ...
  const breadcrumbItems = [
    ...
    { label: language === "ar" ? `السنة الحالية (${currentYear})` : `Current Year (${currentYear})` }
  ]
        ...
        <div className="container mx-auto px-4 pt-24 pb-8">
          ...
          <div className="max-w-4xl mx-auto text-center py-16">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-md mx-auto">
              {language === "ar" 
                ? "حدث خطأ أثناء تحميل نشرات التوعية للسنة الحالية. يرجى المحاولة مرة أخرى."
                : "An error occurred while loading current year awareness bulletins. Please try again."
              }
            </p>
            <Link 
      ...
      <div className="relative z-10 container mx-auto px-4 pt-24 pb-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            {language === "ar" 
              ? "استكشف أحدث نشرات التوعية الأمنية للسنة الحالية"
              : "Explore the latest security awareness bulletins for the current year"
            }
          </p>
        </div>
        ...
        {instructions.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
              {language === "ar" ? "لا توجد نشرات توعية" : "No Awareness Bulletins"}
            </h2>
            <p className="text-gray-600 dark:text-gray-400 mb-8">
              {language === "ar" 
                ? "لم يتم العثور على نشرات توعية للسنة الحالية."
                : "No awareness bulletins found for the current year."
              }
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {instructions.map((instruction, index) => (
              <Link
                key={instruction.id}
                href={`/simple/awareness-years/current/${instruction.id}`}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl border-2 border-slate-200 dark:border-slate-700 shadow-lg hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 hover:scale-[1.02] h-full will-change-transform overflow-hidden">
                  ...
                  <div className="p-6 space-y-4">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 text-sm line-clamp-3">
                      {language === "ar" 
                        ? instruction.description || "نشرة توعية أمنية مهمة"
                        : instruction.descriptionEn || instruction.description || "Important security awareness bulletin"
                      }
                    </p>
