...
export default function TechniquePageClient({ technique, procedureId, controlId, safeguardId }: TechniquePageClientProps) {
  ...
  useEffect(() => {
    const fetchImplementationSteps = async () => {
      try {
        const stepsData = await container.services.procedures.getImplementationStepsByTechniqueId(technique.id, 1, 100)
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-6">
              <Link href={`/procedures/${procedureId}/${controlId}/${safeguardId}`}>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {implementationSteps.map((step) => (
                  <Link key={step.id} href={`/procedures/${procedureId}/${controlId}/${safeguardId}/${technique.id}/${step.id}`}>
