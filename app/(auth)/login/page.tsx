                        ...
                        <div className="relative">
                              ...
                              <div className={`relative backdrop-blur-2xl border rounded-3xl shadow-2xl p-8 md:p-10 ${isDark ? "bg-slate-900/80 border-slate-700/50" : "bg-white/90 border-slate-200"}`}>
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-5">
                                          ...
                                          <div className={`relative flex items-center py-4 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
                                                <div className="flex-grow border-t border-inherit"></div>
                                                <span className="flex-shrink-0 px-4 text-xs font-medium uppercase">{t("auth.or") || "OR"}</span>
                                                <div className="flex-grow border-t border-inherit"></div>
                                          </div>
                                          ...
                                          <Button
                                                ...
                                                onClick={async () => {
                                                      try {
                                                            ...
                                                            const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/Client/Auth/keycloak/login`);
                                                            if (res.ok) {
                                                                  const data = await res.json();
                                                                  if (data.url) {
                                                                        window.location.href = data.url;
                                                                        return; // do not stop loading spinner since we are navigating away
                                                                  }
                                                            }
                                                            console.error("Failed to fetch Keycloak URL");
                                                            setIsKeycloakLoading(false);
                                                      } catch (e) {
                                                            console.error("Error redirecting to Keycloak", e);
                                                            setIsKeycloakLoading(false);
                                                      }
                                                }}
