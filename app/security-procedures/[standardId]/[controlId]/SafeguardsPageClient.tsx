...
import { useSecurityProcedureSafeguards, useSecurityProcedureControl } from "@/core/hooks/use-security-procedures"
...
export default function SafeguardsPageClient({ standardId, controlId }: SafeguardsPageClientProps) {
  const { language, t } = useLanguage()
  const router = useRouter()

  const { control, loading: controlLoading } = useSecurityProcedureControl(controlId)
  const { safeguards, loading: safeguardsLoading, error } = useSecurityProcedureSafeguards(controlId)

  const loading = controlLoading || safeguardsLoading
...
function SafeguardCard({
  ...
  controlId,
  index,
}: { safeguard: any; standardId: string; controlId: string; index: number }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
      <Link href={`/security-procedures/${standardId}/${controlId}/${safeguard.id}`}>
