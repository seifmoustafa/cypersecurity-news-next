...
export default function Breadcrumbs({ items, isLoading = false }: BreadcrumbsProps) {
  ...
  return (
    <nav className="flex items-center space-x-2 rtl:space-x-reverse mb-8">
      ...
      >
        <Home className="h-4 w-4 mr-2 rtl:mr-0 rtl:ml-2" />
        <span className="text-lg font-medium">{language === "ar" ? "الرئيسية" : "Home"}</span>
      </Link>
      ...
      {items.map((item, index) => (
        <div key={index} className="flex items-center space-x-2 rtl:space-x-reverse">
          ...
          {item.href ? (
            <Link 
              href={item.href}
              className="text-lg font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors duration-300"
            >
              {item.label}
            </Link>
          ) : (
