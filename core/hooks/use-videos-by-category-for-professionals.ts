...
interface UseVideosByCategoryForProfessionalsReturn {
  videos: ApiVideo[]
  loading: boolean
  error: string | null
...
export function useVideosByCategoryForProfessionals(
  categoryId: string,
  page: number = 1,
  pageSize: number = 10,
  search?: string
): UseVideosByCategoryForProfessionalsReturn {
  ...
  useEffect(() => {
    ...
    const fetchVideos = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await container.services.media.getVideosByCategoryForProfessionals(categoryId, page, pageSize, search)
