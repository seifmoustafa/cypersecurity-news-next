...
interface UseLecturesByCategoryForProfessionalsReturn {
  lectures: ApiLecture[]
  loading: boolean
  error: string | null
...
export function useLecturesByCategoryForProfessionals(
  categoryId: string,
  page: number = 1,
  pageSize: number = 10,
  search?: string
): UseLecturesByCategoryForProfessionalsReturn {
  ...
  useEffect(() => {
    ...
    const fetchLectures = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await container.services.media.getLecturesByCategoryForProfessionals(categoryId, page, pageSize, search)
